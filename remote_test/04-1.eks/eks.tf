...
resource "aws_eks_cluster" "eks_api" {
  ...
  vpc_config {
    ...
    subnet_ids              = concat(data.terraform_remote_state.vpc.outputs.smp_public_dup_front_subnet_ids, 
                                     data.terraform_remote_state.vpc.outputs.smp_unique_backend_subnet_ids,
                                     data.terraform_remote_state.vpc.outputs.smp_dup_back_subnet_ids)   # (aws_subnet.api_public_dup_front_subnet.*.id, aws_subnet.api_private_unique_backend_subnet.*.id)
  }

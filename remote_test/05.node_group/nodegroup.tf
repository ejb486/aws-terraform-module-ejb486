...
data "aws_ssm_parameter" "eksami" {
  name=format("/aws/service/eks/optimized-ami/%s/amazon-linux-2/recommended/image_id", data.aws_eks_cluster.smp_eks.version)
  depends_on = [
    data.aws_eks_cluster.smp_eks
  ]
}

# getting info already created eks cluster 
data "aws_eks_cluster" "smp_eks" {
  name = data.terraform_remote_state.eks.outputs.smp_cluster_name
}
...
resource "aws_eks_node_group" "mgmt_node" {
    cluster_name    = data.aws_eks_cluster.smp_eks.name
    ...
    labels = {
        "alpha.eksctl.io/cluster-name"   = data.aws_eks_cluster.smp_eks.name
        "alpha.eksctl.io/nodegroup-name" = "${local.project_id}-${local.env}-eks-ng-mgmt-1"
        "role"                           = "management"
        "eks/cluster-name"               = data.aws_eks_cluster.smp_eks.name

...
resource "aws_lb" "network_lb" {
    ...
    subnet_mapping {
        subnet_id     = element(data.terraform_remote_state.vpc.smp_public_dup_front_subnet_ids, 0) #element(aws_subnet.api_public_dup_front_subnet.*.id, 0)
        allocation_id = element(aws_eip.eip_nlb.*.id, 0) 
    }
    subnet_mapping {
        subnet_id     = element(data.terraform_remote_state.vpc.smp_public_dup_front_subnet_ids, 1)

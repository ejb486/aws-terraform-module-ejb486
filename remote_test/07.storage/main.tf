...
terraform {
  backend "s3" {
    bucket 					= "s3-smp-terraform"                            #bucket name
    key         		= "terraform/smp/stg/storage/terraform.tfstate"     #bucket key 
    region 					= "ap-northeast-2"                              #region 
    encrypt 				= true                                          #encrypt yn 
    dynamodb_table 	= "dydb_smp_storage_terraform"                      #dynamodb table for locking
  }
}
...
data "terraform_remote_state" "eks" {
  ...
  config {
    ...
    region = "ap-northeast-2"
    encrypt = true
    lock_table = "dydb_smp_eks_terraform"
    acl = "bucket-owner-full-control"
  }
}
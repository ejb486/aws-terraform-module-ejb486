...
terraform {
  backend "s3" {
    ...
    key         		= "terraform/aurora/terraform.tfstate"    #bucket key 
    region 					= "ap-northeast-2"                     #region 
    encrypt 				= true                                 #encrypt yn 
    dynamodb_table 	= "dydb_tdcsdev_aurora_terraform"         #dynamodb table for locking
  }
}
...
data "terraform_remote_state" "aurora-parameter" {
  backend = "s3"
  config = {
    bucket = "s3-tdcsdev-terraform"
    key = "terraform/aurora-parameter/terraform.tfstate"
    region = "ap-northeast-2"
    encrypt = true
    acl = "bucket-owner-full-control"
    profile="tdcsdev"
